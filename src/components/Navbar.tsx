...
export const Navbar: React.FC<NavbarProps> = ({ page, setPage }) => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-100 flex items-center justify-between px-4 md:px-6 h-[60px] bg-[#0a0a0f]/85 backdrop-blur-xl border-b border-white/5">
            <div onClick={() => setPage('home')} style={{ display: 'flex', alignItems: 'center' }}>
                <Logo />
            </div>

            {/* Desktop Nav */}
            <div className="hidden sm:flex gap-1 md:gap-4 items-center">
            ...
            <div className="flex sm:hidden gap-1 items-center overflow-x-auto no-scrollbar max-w-[150px]">
                <button onClick={() => setPage('docs')} className={`p-2 text-xs ${page === 'docs' ? 'text-white' : 'text-[#8a8a9a]'}`}>Docs</button>
                <button onClick={() => setPage('playground')} className={`p-2 text-xs ${page === 'playground' ? 'text-white' : 'text-[#8a8a9a]'}`}>Live</button>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {/* NPM Badge - Hidden on small mobile */}
                <a
                    href="https://www.npmjs.com/package/react-smart-date-compare"
                    target="_blank"
                    rel="noreferrer"
                    className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-xs text-[#8a8a9a] no-underline font-medium"
                >
                    <span className="text-[#cb3837] font-bold">npm</span>
                    <span>v1.1.1</span>
                </a>
                ...
                <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#6366f1]/15 border border-[#6366f1]/30 text-xs md:text-sm text-[#a5b4fc] no-underline font-medium transition-all hover:bg-[#6366f1]/25"
                >
